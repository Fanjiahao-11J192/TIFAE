...
class SimpleClassifier2(nn.Module):
    ''' Linear classifier, use embedding as input
        Linear approximation, should append with softmax
    '''
    def __init__(self, embd_size, output_dim, dropout=0.3, use_bn=False):
        super(SimpleClassifier2, self).__init__()
        self.dropout = dropout
        self.proj1 = nn.Linear(embd_size, embd_size*4)
        self.proj2 = nn.Linear(embd_size*4, embd_size*2)
        self.proj3 = nn.Linear(embd_size * 2, embd_size)
        self.C = nn.Linear(embd_size, output_dim)
        self.use_bn = use_bn

    def forward(self, x):
        x1 = F.dropout(F.relu(self.proj1(x)),self.dropout)
        x2 = F.dropout(F.relu(self.proj2(x1)),self.dropout)
        x3 = F.dropout(F.relu(self.proj3(x2)), self.dropout)
        x3 += x  # 作残差连接
        output = self.C(x3)
        return output
