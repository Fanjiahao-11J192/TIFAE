...
class ResidualAE(nn.Module):
    ...
    def forward(self, x):
        ...
        recon = self.transition(x_in + x_out)
        # recon_temp = self.Linear1(x_in + x_out)
        # recon_temp2 = self.relu(recon_temp)
        # recon = self.Linear2(recon_temp2)
...
class TE(nn.Module):
    def __init__(self, input_dim,heads_num,layers_num):
        ...
        self.TransformerEncoder = TransformerEncoder(input_dim, heads_num,layers_num)
        # 权重矩阵
    ...
    def forward(self, x):
        enc = self.TransformerEncoder(x) # [length,Batchsize,dim]
        x = x.transpose(0,1)# [Batchsize,length,dim]
        latent = self.SigmoidAttn(x, self.Wr, self.Wm, self.Wu)  # [batch_size,80]
        return enc,latent
...
class DE(nn.Module):
    def __init__(self, input_dim,output_dim,heads_num,layers_num):
        super(DE, self).__init__()
        self.TransformerDecoder = TransformerEncoder(input_dim, heads_num,layers_num)
        # 权重矩阵
        self.Wu = nn.Parameter(torch.randn(input_dim, input_dim))
        self.Wm = nn.Parameter(torch.randn(input_dim, input_dim))
        self.Wr = nn.Parameter(torch.randn(input_dim, 1))
    ...
    def forward(self, x):
        dec = self.TransformerDecoder(x) # [length,Batchsize,dim]
        dec = dec.transpose(0,1)# [Batchsize,length,dim]
        latent = self.SigmoidAttn(dec, self.Wr, self.Wm, self.Wu)  # [batch_size,80]
        output = self.transition(latent)
        return output
    def SigmoidAttn(self,inputs,Wr, Wm, Wu):
        H_hot = torch.mul(torch.matmul(inputs.reshape(-1, inputs.shape[-1]), Wu),
                          torch.sigmoid(torch.matmul(inputs.reshape(-1, inputs.shape[-1]), Wm))) #[batch*length,dim]
