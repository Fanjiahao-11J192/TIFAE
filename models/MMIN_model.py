...
class MMINModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--image_dir', type=str, default='./MMIN_image', help='models image are saved here')
        return parser
    ...
    def forward(self):
        ...
        if self.isTrain:
            with torch.no_grad():
                self.T_embd_A,self.T_embd_A_NoMaxPool = self.pretrained_encoder.netA(self.A_reverse)
                self.T_embd_L,self.T_embd_L_NoMaxPool = self.pretrained_encoder.netL(self.L_reverse)
                self.T_embd_V,self.T_embd_V_NoMaxPool = self.pretrained_encoder.netV(self.V_reverse)
