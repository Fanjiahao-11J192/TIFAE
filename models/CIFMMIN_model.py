...
class CIFMMINModel(BaseModel):
    ...
    def forward(self):
        ...
        self.feat_A_miss,_ = self.netA(self.A_miss)  # missing modaltity feature
        ...
        self.feat_A_consistent,_ = self.netConA(self.A_miss)
        self.feat_L_consistent,_ = self.netConL(self.L_miss)
        self.feat_V_consistent,_ = self.netConV(self.V_miss)
        ...
        if self.isTrain:
            with torch.no_grad():
                ...
                embd_A_consistent,_ = self.pretrained_encoder.netConA(self.acoustic)
                embd_L_consistent,_ = self.pretrained_encoder.netConL(self.lexical)
                embd_V_consistent,_ = self.pretrained_encoder.netConV(self.visual)
