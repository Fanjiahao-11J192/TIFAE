...
from models.pretrained_TIFAE_model import UttFusionModel
...
class MTMMIN5Model(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.model_names.append('A_EM_L')
        self.model_names.append('A_EM_V')
        self.model_names.append('V_EM_A')
        self.model_names.append('V_EM_L')
        self.model_names.append('L_EM_A')
        self.model_names.append('L_EM_V')
        self.netA_EM_L = MEM(opt.conv_dim_a,1,1).to(self.device)
        self.netA_EM_V = MEM(opt.conv_dim_a, 1, 1).to(self.device)
        self.netV_EM_A = MEM(opt.conv_dim_v,1,1).to(self.device)
        self.netV_EM_L = MEM(opt.conv_dim_v, 1, 1).to(self.device)
        self.netL_EM_A = MEM(opt.conv_dim_l, 1, 1).to(self.device)
        self.netL_EM_V = MEM(opt.conv_dim_l, 1, 1).to(self.device)
        if self.isTrain:
